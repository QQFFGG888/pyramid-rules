...
概念之间还有一条依赖闭合：讲解某个概念时，只能引用排在它前面且已讲完的前置概念，不得引用后面才讲的高级术语。
...
第一步跑脚本，查机械项。AI 套话、感叹号、省略号、修订说明类词句、写作过程的话、五级标题、章末勾选框，全部要求为 0。理解成本也在这里查：单句字数、分句数、并列项数按层设上限，超了判为不通过。
...
```
pyramid-tutorial/
├── SKILL.md                    # 技能主文件：触发条件、三条立意、依赖闭合、生成流程、输出格式
├── references/
