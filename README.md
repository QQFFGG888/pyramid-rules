...
概念单元由四段组装，顺序固定。原文段（有门槛的原文，如有）→ 精读段（技术解释）→ 浅读段（通俗落回）→ 收尾段（小结、实操与承接）。术语首现处标编号，文末速查表收口。

概念与概念之间还有一条依赖闭合：讲一个概念时，只能引用排在它前面、且已讲完的前置概念，不许引用后面才讲的高级术语。口诀是先讲该有的，再讲要讲的。
...
第一步跑脚本，查机械项。AI 套话、感叹号、省略号、修订说明类词句、写作过程的话、五级标题、章末勾选框，全部要求为 0。理解成本也在这里查：单句字数、分句数、并列项数按层设上限，超了就是 FAIL。

```
bash scripts/selfcheck.sh 产出文件.md
```

第二步核对人工项。L1 零术语、小白复读、不引入新词解释旧词、比喻一致、要素完整、因果链闭合、自然引导、依赖闭合、用词通俗、四段顺序与术语编号。判读标准见 `references/selfcheck.md`。
...
```
pyramid-tutorial/
├── SKILL.md                    # 技能主文件：触发条件、三层立意、依赖闭合、生成流程、输出格式
├── references/
...
v5.1。新增三条立意、依赖闭合、读者起点探测、理解成本阈值与用词通俗规则；自检脚本增理解成本检查四项；单元四带改名单元四段，旧词熵增、名词锚、元叙述全部换成直白说法。此前版本：v5 单元四带落地，术语编号锚、章末名词与文末四列速查表；v4 目录结构升级，示例迁至 examples/，自检配方落成 scripts/selfcheck.sh；v3.1 规则措辞与默认值精修；v3 四层金字塔与十条规则边界成型。
