...
v5.1。新增三条立意、前置概念顺序规则、读者起点判断、阅读负担阈值与用词通俗规则；自检脚本增阅读负担检查四项；单元四带改名单元四段，此前生造的压缩词与比喻式命名全部换成直白说法。此前版本：v5 单元四带落地，术语编号、章末名词与文末四列速查表；v4 目录结构升级，示例迁至 examples/，自检清单写成 scripts/selfcheck.sh；v3.1 规则措辞与默认值精修；v3 四层金字塔与十条规则边界成型。
